...
public class LeftClickSkill extends CooldownSkill {
    ...
    public LeftClickSkill() {
        this(CONSUMABLE_DEFAULT, DEFAULT_SLOTS.stream().map(Slot::clone).collect(Collectors.toSet()), Lists.newArrayList(),
                new CooldownOptions(), Lists.newArrayList(), Lists.newArrayList(), RAY_CAST_DISTANCE_DEFAULT);
    }

    public LeftClickSkill(boolean consumable, Set<Slot> allowedSlots, List<Action> onItemActions,
