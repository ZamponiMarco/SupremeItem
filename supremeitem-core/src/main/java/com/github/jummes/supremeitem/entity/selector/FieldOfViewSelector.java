...
public class FieldOfViewSelector extends EntitySelector {
    ...
    public Predicate<LivingEntity> getAbstractFilter(Source source, Target target) {
        return entity -> {
            ...
            Vector minDirection = e.getLocation().getDirection().clone().setY(0).rotateAroundY(-Math.toRadians(fov / 2)).normalize();
            Vector maxDirection = e.getLocation().getDirection().clone().setY(0).rotateAroundY(Math.toRadians(fov / 2)).normalize();
            Vector toCheck = entity.getLocation().clone().subtract(e.getLocation()).toVector().setY(0).normalize();
